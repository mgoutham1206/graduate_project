...
<script>
	...
	function activationClick() {
		var studentID = document.getElementById("userinfoId").value;
		//alert("going to "+ window.location.pathname.substring(0,window.location.pathname.indexOf('/activate', 0))+ "/ds/activateStudent/" + studentID)
		$
				.ajax({
					url : window.location.pathname.substring(0,
							window.location.pathname.indexOf('/activate', 0))
							+ "/ds/activateStudent/" + studentID,
					type : "GET",
					...
					success : function(data) {
						//alert(data)
						if (data) {
							document.getElementById("message").innerHTML = "Your enrollment has been successfully activated. <br> <a href="+window.location.pathname.substring(0,window.location.pathname.indexOf('/stu', 0))+'/view/login'+">Click here to go to Login Page</a>";
						} else {
							document.getElementById("message").innerHTML = "Not Activated. Please check with the Instructor.";
						}
					},
					complete : function() {
...
<body class="hold-transition skin-green sidebar-mini">
	<div class="wrapper">
		...
		<header class="main-header">
			...
			<!-- Logo -->
			<a href="${pageContext.request.contextPath}/view/login" class="logo" id="geoapplogo"
				...
				<div class="container">
					<div class="row">
						...
						<div class="col-md-12">
							...
							<br> <br> <br>
							<c:set var="studentLoginName" value="${studentInfo.loginName}"
								scope="page" />
							<c:set var="studentId" value="${studentInfo.userID}" scope="page" />
							<c:set var="studentEmail" value="${studentInfo.emailAddress}"
								scope="page" />
							<c:set var="studentName" value="${studentInfo.name}" scope="page" />
							<input type="hidden" id="userinfoId"
								value='<c:out value="${studentId}"></c:out>' />
							<!-- <script>
								var studentID = $
								{
									studentId
							...
							<p>
								Hi <b> <c:out value="${studentLoginName}" />
								</b>
							</p>
							<p>Please check your information and click on activate button</p>
							<p>
								Email : <b><c:out value="${studentEmail}" /> </b>
							</p>
							<p>
								Login Name : <b><c:out value="${studentLoginName}" /> </b>
							</p>
							<p>
								Name : <b><c:out value="${studentName}" /> </b>
							</p>
							<br> <br>

							<button class="btn btn-primary btn-md" id="activeButtonId"
								onclick="activationClick()">Activate</button>
	...
	<footer class="main-footer">
		<!-- Default to the left -->
		<strong>GeoApp © 2015<a href="http://www.nwmissouri.edu/">
				NWMSU</a>.
